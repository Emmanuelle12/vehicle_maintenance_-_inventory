...
export default function Edit({ params }: { params: { slug: string } }) {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        toast.promise(
            axios.put(`/api/staffs/edit?staff_id=${params.slug}`, updatedStaffForm),
            {
                pending: 'Updating staff...',
                success: 'Staff created updated.',
                error: {
    ...
    const getStaff = useCallback(async () => {
        await axios.get(`/api/staffs/edit?staff_id=${params.slug}`)
        .then(response => {
            const sf = response.data?.staff
            setStaffForm(sf)
        })
        .catch(error => {
            console.log(error)
